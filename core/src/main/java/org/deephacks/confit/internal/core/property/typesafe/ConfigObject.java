 ...
 *
 * <p>
 * Like list {@link ConfigValue} subtypes, {@code ConfigObject} is immutable.
 * This makes it threadsafe and you never have to create "defensive copies." The
 * mutator methods from {@link java.util.Map} list throw
 ...
 * {@code lookup()} returns Java's null then the key was not present in the parsed
 * file (or wherever this value tree came from). If {@code lookup()} returns a
 * {@link ConfigValue} with type {@code ConfigValueType#NULL} then the key was
 * set to null explicitly in the typesafe file.
 *
 * <p>
 * <em>Do not implement {@code ConfigObject}</em>; it should only be implemented
 * by the typesafe library. Arbitrary implementations will not work because the
...
public interface ConfigObject extends ConfigValue, Map<String, ConfigValue> {
    ...
    /**
     * Clone the object with only the given key (and its children) retained; list
     * sibling keys are removed.
     *
     * @param key
     *            key to keep
     * @return a copy of the object minus list keys except the one specified
     */
