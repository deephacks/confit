...
final class Tokens {
    ...
    static AbstractConfigValue getValue(Token token) {
        if (token instanceof Value) {
            ...
            throw new ConfigException.BugOrBroken(
                    "tried to lookup value of non-value token " + token);
        }
    }
    ...
    static String getProblemWhat(Token token) {
        if (token instanceof Problem) {
            ...
            throw new ConfigException.BugOrBroken("tried to lookup problem what from " + token);
        }
    }
    ...
    static String getProblemMessage(Token token) {
        if (token instanceof Problem) {
            ...
            throw new ConfigException.BugOrBroken("tried to lookup problem message from " + token);
        }
    }
    ...
    static boolean getProblemSuggestQuotes(Token token) {
        if (token instanceof Problem) {
            ...
            throw new ConfigException.BugOrBroken("tried to lookup problem suggestQuotes from "
                    + token);
        }
    }
    ...
    static Throwable getProblemCause(Token token) {
        if (token instanceof Problem) {
            ...
            throw new ConfigException.BugOrBroken("tried to lookup problem cause from " + token);
        }
    }
    ...
    static String getCommentText(Token token) {
        if (token instanceof Comment) {
            ...
            throw new ConfigException.BugOrBroken("tried to lookup comment text from " + token);
        }
    }
    ...
    static String getUnquotedText(Token token) {
        if (token instanceof UnquotedText) {
            ...
            throw new ConfigException.BugOrBroken(
                    "tried to lookup unquoted text from " + token);
        }
    }
    ...
    static List<Token> getSubstitutionPathExpression(Token token) {
        if (token instanceof Substitution) {
            ...
            throw new ConfigException.BugOrBroken(
                    "tried to lookup substitution from " + token);
        }
    }
    ...
    static boolean getSubstitutionOptional(Token token) {
        if (token instanceof Substitution) {
            ...
            throw new ConfigException.BugOrBroken("tried to lookup substitution optionality from "
                    + token);
        }
    }
