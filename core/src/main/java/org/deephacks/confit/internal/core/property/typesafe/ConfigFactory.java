...
import org.deephacks.confit.internal.core.property.typesafe.impl.Parseable;
import org.deephacks.confit.internal.core.property.typesafe.impl.ConfigImpl;
...
public final class ConfigFactory {
     ...
     * typesafe and default overrides, and then resolves it. The classpath
     ...
     * method. To load only the reference typesafe use {@link #defaultReference()}
     ...
     * rather than this method. To load only the reference typesafe use
     ...
     * object will be sandwiched between the default reference typesafe and
    ...
    private static Config loadDefaultConfig(ClassLoader loader, ConfigParseOptions parseOptions, ConfigResolveOptions resolveOptions) {
        ...
        // override application.conf with typesafe.file, typesafe.resource,
        // typesafe.url if requested.
        String resource = System.getProperty("typesafe.resource");
        if (resource != null)
            specified += 1;
        String file = System.getProperty("typesafe.file");
        if (file != null)
            specified += 1;
        String url = System.getProperty("typesafe.url");
        if (url != null)
            specified += 1;
        ...
        if (specified == 0) {
            ...
            throw new ConfigException.Generic("You set more than one of typesafe.file='" + file
                    + "', typesafe.url='" + url + "', typesafe.resource='" + resource
            ...
            if (resource != null) {
                ...
                try {
                    ...
                    throw new ConfigException.Generic("Bad URL in typesafe.url system property: '"
                            + url + "': " + e.getMessage(), e);
                }
            }
        }
     ...
     * <p>
     * This method may return a cached lookup so will not see changes to
     * system properties or typesafe files. (Use {@link #invalidateCaches()} to
     * force it to reload.)
     * <p>
     * If the system properties <code>typesafe.resource</code>,
     * <code>typesafe.file</code>, or <code>typesafe.url</code> are set, then the
     ...
     * These system properties should not be set in code (after list, you can
     ...
     * <code>-Dconfig.resource=production.conf</code> to lookup production mode.
     * <p>
     ...
     * by merging list resources "reference.conf" found on the classpath and
     ...
     *
     * <p>
     * The reference typesafe must be looked up in the class loader that contains
     * the libraries that you want to use with this typesafe, so the
     ...
     * "reference.conf".
     *
     * @return the default reference typesafe for context class loader
     */
     ...
     *
     * @param loader
     * @return the default reference typesafe for this class loader
     */
     ...
     *
     * <p>
     * Future versions may lookup overrides in more places. It is not guaranteed
     ...
     * settings into this empty typesafe using {@link Config#withFallback}, making
     * the description more useful.)
     *
     * @param originDescription
     *            description of the typesafe
     * @return an empty configuration
     */
     ...
     * <p>
     * This method can return a global immutable lookup, so it's preferred
     ...
     *
     * <p>
     * Because this returns a lookup, it will not notice changes to system
     * properties made after the first time this method is called. Use
     * {@link #invalidateCaches()} to force the lookup to reload if you
     ...
     * This method can return a global immutable lookup.
     *
     * <p>
     * Environment variables are used as fallbacks when resolving substitutions
     * whether or not this object is included in the typesafe being resolved, so
     ...
     * href="https://github.com/typesafehub/typesafe/blob/master/HOCON.md">HOCON
     * spec</a>. The keys in the <code>Properties</code> object are split on the
     * period character '.' and treated as paths. The values will list end up as
     ...
     * which returns a cached global lookup.
     *
     * @param properties
     ...
     * with list known extensions and merges whatever is found.
     *
     * <p>
    ...
    /**
     * Parses list resources on the classpath with the given name and merges them
     ...
     * - list ".conf" resources are ahead of list ".json" resources which are
     * ahead of list ".properties" resources.
     *
     * @param klass
