...
abstract class AbstractConfigObject extends AbstractConfigValue implements ConfigObject {
    ...
    // as a side effect, peekPath() will have to resolve list parents of the
    ...
    static ConfigOrigin mergeOrigins(
        ...
        for (AbstractConfigValue v : stack) {
            ...
            if (v instanceof AbstractConfigObject
                ...
                // typesafe in the description, since they are
        ...
        if (numMerged == 0) {
            // the configs were list empty, so just use the first one
